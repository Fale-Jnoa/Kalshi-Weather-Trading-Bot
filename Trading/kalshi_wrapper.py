...
_PRIV_KEY_ENV = os.getenv("KALSHI_PRIVATE_KEY", "")
...
def _get(path: str, params: dict | None = None, demo: bool = False) -> dict:
    ...
    r = requests.get(base + path, headers=headers, params=params, timeout=12)
    if not r.ok:
...
def _post(path: str, body: dict, demo: bool = False) -> dict:
    ...
    r = requests.post(base + path, headers=headers, json=body, timeout=12)
    if not r.ok:
